...
// Logos SVG/texto de cada pasarela
function Logo({ pasarela }) {
  // Usamos texto estilizado como Komercia cuando no hay imagen
  const estilos = {
            ...
            <div style={g.actionsConectada}>
              ...
              <div style={g.accionesBtns}>
                ...
                {confirmDelete ? (
                  ...
                  <button onClick={() => setConfirmDelete(true)} style={g.btnEliminar}>✕</button>
                )}
              </div>
            </div>
          ) : (
          ...
          {estaConectada ? (
            <>
              ...
              {confirmDelete ? (
                ...
                <button onClick={() => setConfirmDelete(true)} style={l.btnEliminar}>✕</button>
              )}
            </>
          ) : (
            <>
              ...
              <button
                onClick={() => setExpandida(v => !v)}
                style={{ ...l.btnConectar, backgroundColor: pasarela.colorBrand }}
              >
                {expandida ? "Cancelar" : "Conectar →"}
              </button>
            </>
          )}
        </div>
...
const g = {
  ...
  btnEliminar: { background: "none", border: "none", color: "#cbd5e1", cursor: "pointer", fontSize: 12, marginLeft: "auto" },
...
const l = {
  ...
  logoWrap: {
    ...
    display: "flex", alignItems: "center", justifyContent: "center",
    backgroundColor: "#fafbfc", borderRadius: 8, padding: "0 10px",
  },
  ...
  btnEliminar: { background: "none", border: "none", color: "#cbd5e1", cursor: "pointer", fontSize: 13 },
