...
// Permisos fijos solo para roles base (fallback si la API falla)
const PERMISOS_BASE = {
...
export default function DashboardEmpresa() {
  ...
  useEffect(() => {
    ...
    if (ROLES_FIJOS.includes(rolId)) {
      // Roles base: usar permisos hardcodeados directamente
      setPermisosRol(PERMISOS_BASE[rolId] || []);
      setCargandoPermisos(false);
      return;
    }

    // Rol personalizado: cargar permisos desde la API
  ...
  // Menú filtrado según permisos cargados
  ...
  // Cuando los permisos carguen, establecer la primera sección disponible
  useEffect(() => {
    if (menu.length > 0 && !seccion) {
      setSeccion(menu[0].key);
    }
  }, [menu]);
  ...
  const irA = (key) => {
    if (permisosRol?.includes(key)) setSeccion(key);
  };

  const rolLabel = rolId === 1 ? "SuperAdmin" : rolId === 2 ? "Administrador" : rolId === 3 ? "Empleado" : rolId === 4 ? "Cliente" : "Usuario";
  const inicial  = nombreUsuario.charAt(0).toUpperCase();

  // ── Pantalla de carga mientras se obtienen permisos ──────────────────────
  if (cargandoPermisos) {
    return (
      <div style={{
        display: "flex", alignItems: "center", justifyContent: "center",
        minHeight: "100vh", background: "#F0F4F8",
        fontFamily: "'Sora', sans-serif", flexDirection: "column", gap: 12,
      }}>
        <div style={{
      ...
      <div className="fw-shell">
        ...
        <aside className="fw-sidebar">
          ...
          <nav className="fw-sb-nav">
            {/* Sección Principal */}
            {menu.some(i => ["productos", "clientes", "ventas", "reportes"].includes(i.key)) && (
